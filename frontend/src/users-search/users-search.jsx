...
export const UsersSearch = ({value: loginPredicate, onChange: onChangeProp}) => {
    const [predicate, setPredicate] = useState(loginPredicate || '')
    const { users } = useSelector((state) => console.log({state}) || state.usersReducer);
    ...
    useEffect(() => {
        ...
        setPredicate(loginPredicate)}, [loginPredicate])
    ...
    const fetchUsers = async (predicate) => {
        ...
        try {
            const res = await dispatch(fetchUsersThunk({loginPredicate: predicate})).unwrap();
            if (res.error) {
